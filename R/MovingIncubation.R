...
Taillexxx <- diff(serie.taille[indice.x1:indice.x2])
Tempxxx <- nids[indice.x1:(indice.x2-1), "Temperatures C"]
STRN <- .SSM(Tempxxx, SexualisationTRN)[[1]]

    weighted.mean.temp.TSP <- sum(Taillexxx*Tempxxx)/sum(Taillexxx)
    TSD.mean.temp.TSP <- sum(Timexxx*Tempxxx*STRN)/sum(Timexxx*STRN)
    }
		if (y>=	mean.ts) {
...
# fin du test pour savoir si l'incubation est suffisamment longue
  ...
  rep.TSDTEMPERATURE <- c(rep.TSDTEMPERATURE, TSD.mean.temp.TSP)
  if (is.na(meanduree)) {
...
TSDTEMPERATURE <- c(TSDTEMPERATURE, mean(rep.TSDTEMPERATURE, na.rm = TRUE))
...
SDTSDTEMPERATURE <- c(SDTSDTEMPERATURE, sd(rep.TSDTEMPERATURE, na.rm = TRUE))
...
return(data.frame(time=incubationTIME, sd.time=SDincubationTIME, 
  ...
  TSD.incubation.Temperature=TSDTEMPERATURE, 
  sd.TSD.incubation.Temperature=SDTSDTEMPERATURE))

...
#' @param WAIC Should WAIC data be returned?
...
info.nests <- function(x=NULL                                                  , 
  ...
  papply <- ifelse(parallel, getOption("mc.cores", parallel::detectCores()), 1)
  
  if (inherits(x, "NestsResult")) NestsResult <- x
  if (inherits(x, "numeric")) parameters <- x
  ...
  temperatures_ec <- lapply(temperatures$Nests, FUN = function(x) x$data)
  
  # Je ne garde que les series que je dois travailler
  temperatures_ec <- temperatures_ec[series]
  ...
  AnalyseTraces <- universalmclapply(progressbar=progressbar, mc.cores = papply, X=1:nrow(df_random), 
                                     FUN=function (ligne_df_random) {
                                       ...
                                       if (length(temperature.heterogeneity) == 1) {
                                         tht <- 0
                                         if (temperature.heterogeneity != 0) {
  ...
  if (out=="likelihood") {
    # Je dois appliquer les weights
    # LnLx <- sum(sapply(AnalyseTraces, FUN = function(x) x$likelihood))
    LnLx <- sapply(AnalyseTraces, FUN = function(x) x$likelihood)
    LnLx <- LnLx*weight[name]
    sLnLx <- sum(LnLx)
    if (WAIC) {
      attributes(sLnLx) <- list(WAIC=-unname(LnLx))
    }
    return(sLnLx)
  }
  ...
  if (!stop.at.hatchling.metric) {
    ...
    for (serie_en_cours in unique(series_list)) {
      ...
      df_list_ec <- df_list[names(df_list) == serie_en_cours]
    ...
    # La je suis en stop.at.hatchling.metric
    ...
    for (serie_en_cours in unique(series_list)) {
      ...
      # if (length(t_ec) != nrow(metric_ec)) print(serie_en_cours)
      df_TC <- df_list_ec[[1]]
      df_TC[nrow(df_TC), "DeltaT"] <- 1
      df_TC <- df_TC[(df_TC$Time %in% t_ec) & ((df_TC$DeltaT != 0) | (is.na(df_TC$R))), ]
      df_TC[nrow(df_TC), "DeltaT"] <- 0

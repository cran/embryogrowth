...
#  x <- resultNest_4p; xlim=c(0,70); ylimT=c(22, 32); ylimS=c(0,45); series=1; resultmcmc = result_mcmc_4p; embryo.stages="Caretta caretta.SCL"; replicate.CI = 100
  ...
  if (!is.null(resultmcmc)) SE <- FALSE
  ...
  if ((is.null(SE) & is.null(resultmcmc)) | !replicate.CI | replicate.CI<2 | !show.CI) {
    show.CI <- FALSE
    replicate.CI <- 1
  }

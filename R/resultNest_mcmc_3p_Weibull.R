#' Result of the mcmc using the nest database using Weibull function
#' @title Result of the fit using the nest database using Weibull function
#' @author Marc Girondot \email{marc.girondot@@u-psud.fr}
#' @docType data
#' @name resultNest_mcmc_3p_Weibull
#' @encoding UTF-8
...
#' @keywords datasets
#' @usage resultNest_mcmc_3p_Weibull
#' @examples
#' \dontrun{
...
#' pMCMC <- TRN_MHmcmc_p(resultNest_3p_Weibull, accept = TRUE)
#' pMCMC["scale", "Max"] <- pMCMC["scale", "Prior2"] <- 600
#' resultNest_mcmc_3p_Weibull <- GRTRN_MHmcmc(result = resultNest_3p_Weibull, 
#'          adaptive = TRUE,
#'          n.iter = 50000, parametersMCMC = pMCMC, 
#'          intermediate = 1000, filename = "intermediate_mcmc.Rdata")
#' 1-rejectionRate(as.mcmc(resultNest_mcmc_3p_Weibull))
#' as.parameters(resultNest_mcmc_3p_Weibull)
#' layout(mat=matrix(1:4, nrow = 2))
#' plot(resultNest_mcmc_3p_Weibull, parameters = "all", scale.prior = TRUE, las = 1)
#' layout(mat=1)
#' plotR(resultNest_3p_Weibull, resultmcmc=resultNest_mcmc_3p_Weibull, ylim=c(0,4), 
#'        main="Weibull function", show.hist=TRUE, show.density=TRUE)
#' }

...
#' w <- weightmaxentropy(temperatures=formated, control_plot=list(xlim=c(20,36)))
...
weightmaxentropy <- function(temperatures=stop('Temperature data must be provided !') , 
  ...
  pec <- NULL
  for(i in temperatures$Names) {
    pec <- c(pec, par[i])
  }
  
  par <- pec[1:(NbTS-1)]/pec[NbTS]
  
  vm <- setdiff(temperatures$Names, names(par))
  ...
  for(series in temperatures$Names) {
    
    nids <- temperatures$Nests[[series]]$data
  ...
  print(paste("Initial entropy is", getFromNamespace(".weightmaxentropy_fit", ns="embryogrowth")(par, pi=pi, entropy.method=entropy.method, pentropy=control_entropy, vm=vm)))
  
  
  result  <- optim(par, getFromNamespace(".weightmaxentropy_fit", ns="embryogrowth"), pi=pi, entropy.method=entropy.method, pentropy=control_entropy, vm=vm, control=control_optim, method="BFGS", hessian=FALSE)
  
  if (result$convergence==1) {
    print("Maximum number of iterations has been reached. Try increase maxit in controls")
  }
  
  par <- setNames(abs(c(result$par, 1)), temperatures$Names)
